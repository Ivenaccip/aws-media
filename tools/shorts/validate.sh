...
for file in "$OUTPUT_DIR"/short_*.mp4; do
    ...
    width=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=width -of csv=p=0 "$file" 2>/dev/null | tr -d "," || echo "0")
    height=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=height -of csv=p=0 "$file" 2>/dev/null | tr -d "," || echo "0")
    if [ "$width" != "1080" ] || [ "$height" != "1920" ]; then
        ISSUES+=("Resolution is ${width}x${height}, expected 1080x1920")
    fi

    # 3. Check audio track exists
    audio_codec=$(ffprobe -v quiet -select_streams a:0 -show_entries stream=codec_name -of csv=p=0 "$file" 2>/dev/null | tr -d "," || echo "")
    ...
    duration=$(ffprobe -v quiet -show_entries format=duration -of csv=p=0 "$file" 2>/dev/null | tr -d "," || echo "0")
    ...
    size_mb=$(awk "BEGIN{printf \"%.1f\", $file_size/1048576}")  # awk, no bc (Windows)
    ...
    video_codec=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 "$file" 2>/dev/null | tr -d "," || echo "")
