...
# Detect GPU for NVENC (adaptación video-stack: prueba FUNCIONAL — que el encoder
# esté listado no basta cuando el driver es más viejo que el ffmpeg)
HAS_NVENC="false"
if command -v nvidia-smi &>/dev/null && command -v ffmpeg &>/dev/null; then
    if ffmpeg -y -loglevel error -f lavfi -i "color=black:size=64x64:rate=30:duration=0.1" \
...
for input_file in "$INPUT_DIR"/short_*.mp4; do
    ...
    for plat in "${platforms[@]}"; do
        ...
        if [ -f "$output_file" ] && [ "$FORCE" != "true" ]; then
            ...
            size_mb=$(awk "BEGIN{printf \"%.1f\", $size/1024}")  # awk, no bc (Windows)
