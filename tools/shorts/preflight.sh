...
# Check Python venv (adaptación video-stack: también acepta el venv del repo)
...
# Check Remotion node_modules (adaptación video-stack: remotion/ vive en la raíz
# del repo, dos niveles arriba de tools/shorts/)
if [ ! -d "$REPO_ROOT/remotion/node_modules" ]; then
    ERRORS+=("Remotion dependencies not installed — run: cd remotion && npm ci")
fi
